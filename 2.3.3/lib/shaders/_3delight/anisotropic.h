...
#ifndef __anisotropic_h
#define __anisotropic_h
...
begin inputs
	...
	float spreadY
	float roughness
	float fresnelRefractiveIndex
	color specularColor
	float reflectivity
	color reflectedColor
	uniform float anisotropicReflectivity
...
begin shader_extra_parameters stangent
	varying vector stangent = 0; // for anisotropic node
...
maya_anisotropic(
	...
	float i_angle, i_spreadX, i_spreadY, i_roughness, i_fresnelIndex;
	color i_specularColor;
	float i_reflectivity;
	color i_reflectedColor;
	uniform float i_anisotropicReflectivity;
...
)
{
	...
	vector xdir = rotate(
		vdir, -i_angle * PI / 360, point(0,0,0), point(i_normalCamera));
	xdir = normalize( xdir );
	color Is = i_specularColor * getAnisotropic(
		Nn, -In, xdir, i_fresnelIndex, i_roughness,
		i_spreadX, i_spreadY, 0, 0);

	float reflectivity = i_reflectivity;

	if( i_anisotropicReflectivity != 0 )
	{
		// TODO: Implement this.
		reflectivity = 0;
	}
	...
	color Ir = getReflection(
		Nf, I, i_specularColor, reflectivity, i_reflectedColor,
	...
	if( isoutput(aov_specular_no_shadow) )
	{
		aov_specular_no_shadow +=
			...
			getAnisotropic(
				Nn, -In, xdir, i_fresnelIndex, i_roughness,i_spreadX, i_spreadY, 0, 1);
	}
#endif
#ifdef USE_AOV_aov_specular_intensity
	...
	if( isoutput(aov_specular_intensity) )
	{
		aov_specular_intensity +=
			__transparency *
			getAnisotropic(
				Nn, -In, xdir, i_fresnelIndex, i_roughness, i_spreadX, i_spreadY, 0, 0);
	}
#endif
#ifdef USE_AOV_aov_specular_intensity_no_shadow
	...
	if( isoutput(aov_key_lights_specular_intensity) )
	{
		aov_key_lights_specular_intensity +=
			__transparency *
			getAnisotropic(
				Nn, -In, xdir, i_fresnelIndex, i_roughness, i_spreadX, i_spreadY, 1, 0);
	}
#endif
#ifdef USE_AOV_aov_key_lights_specular_intensity_no_shadow
	...
	if( isoutput(aov_key_lights_specular_intensity_no_shadow) )
	{
		aov_key_lights_specular_intensity_no_shadow +=
			__transparency *
			getAnisotropic(
				Nn, -In, xdir, i_fresnelIndex, i_roughness,	i_spreadX, i_spreadY, 1, 1);
	}
#endif
#ifdef USE_AOV_aov_surface_color
  ...
  aov_facing_ratio = abs( normalize(i_normalCamera).normalize(-I) );
	...
	if( isoutput(aov_color_no_shadow) )
	{
		...
		color Is_no_shw = i_specularColor * getAnisotropic(
			Nn, -In, xdir, i_fresnelIndex, i_roughness,
			i_spreadX, i_spreadY, 0, 1);
